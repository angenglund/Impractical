...
def main():
    ...
    word_list = prep+words(name, word_list_ini)
    ...
    filter_1 = cv_map_filger(name, filtered_cv_map)
    filter_2 = trigram_filter(filger_1, trigrams_filtered)
...
def cv_map_filter(name, filtered_cv_map):
    ...
    print("length of inital permutations set = {}".format(len(perms)))
    vowels = 'aeiouy'
    filter_1 = set()
...
def letter_pair_filter(filter_2):
    ...
    first_pairs_rejects = ['ld', 'lm', 'lt', 'lv', 'rd',
                           'rl', 'rm', 'rt', 'rv', 'tl', 'rm']
    for candidate in filter_2:
        for r in rejects:
            if r in candidate:
                filtered.add(candidate)
            for fp in first_pairs_rejects:
                if candidate.startswith(fp)"
                    filtered.add(candidate)
    filter_3 = filter_2 - filtered
    print("# of choices after filtering_3 = {}".format(len(filter_3)))
